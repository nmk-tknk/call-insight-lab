...
> 参考資料(2026-09-22 に docs/reference へ移動)。本書は 2026-09-13 時点の調査記録であり、研究・商用製品の動向と参考文献のために残す。本書の推奨部分のうち、人手ゴールデンセットと PPI 補正(§0-2, §3.4, §5.3)は人手ラベリング不可のため撤回、小型モデルへの蒸留(§3.5)は v1 の費用論点に格下げ、v0 スキーマ案(§5.2)は v0.4 で置き換え済み。現行の原則は docs/01_concept.md、スキーマは docs/02_schema.md、決定の経緯は docs/history.md を参照。本書の推奨(発見ループ、探索層、ドリフト監視)は決定ではなく、決定を経ずに本文へ入れない(docs/01_concept.md §4)。本文は移動時に変更していない。
...
> 注(2026-09-20): 本節の PPI 補正は、02 で人手ラベリングが不可と判明したため撤回した(品質担保は 02 §4 の人手ラベルなしの方式に置き換え)。蒸留は当面見送り、v1 のコスト論点として [11](11_cost_model.md) で扱う。
