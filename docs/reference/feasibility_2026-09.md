...
> 参考資料(2026-09-22 に docs/reference へ移動)。本書は 2026-09-13 時点の調査記録であり、研究・商用製品の動向と参考文献のために残す。本書の推奨部分のうち、人手ゴールデンセットと PPI 補正(§0-2, §3.4, §5.3)は人手ラベリング不可のため撤回、小型モデルへの蒸留(§3.5)は v1 の費用論点に格下げ、v0 スキーマ案(§5.2)は v0.4 で置き換え済み。現行の原則は docs/01_concept.md、スキーマは docs/02_schema.md、決定の経緯は docs/decisions/(0003、0007)を参照。本文は移動時に変更していない。
