...
- 出所: 旧 docs/study/04(語彙の自動進化)全体、12 §0.4(語彙の 3 層)、13 §4(業務知識表)、10 §7(対象マスタ草案)を統合(2026-09-22)。原則は [01_concept.md](01_concept.md) 原則 1・2。
- **決定と案**: この文書で決定なのは [01_concept.md](01_concept.md) M10(語彙依存の構造化は汎用性を失う)と M11(語彙に該当がないはスキーマの失敗)だけ。§2 の語彙進化ループ、§3 の業務知識表の保守、§4 の対象マスタ草案は設計者の案。とくに §2 は「語彙に該当がない」を常態とする前提に立っており、M11 と衝突する。扱いは未決([open_issues.md](open_issues.md) S17)。
...
| 層 | 例 | 業種依存 | 変わり方 |
|---|---|---|---|
| 発話行為の種類 | 照会する、主張する、理由を述べる、了承する、訂正した、暴言 | しない | 固定(役割の一覧そのもの)。問いの有無にかかわらず常に全種類を抽出する(案。[open_issues.md](open_issues.md) A7) |
