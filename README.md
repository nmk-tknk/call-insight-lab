...
構造で覆えない問いはスキーマの不備として塞ぎ、問いに答えるために原文へ LLM を当て直すことはしない(06 原則 5)。
...
- [docs/study/11_cost_model.md](docs/study/11_cost_model.md)
  コストモデル。費用が発生する 5 工程(初回抽出、不備修正の差分再実行、語彙進化、品質ゲート、合成データ)を通話 1 件あたりのトークン数と月次の回数で式にし、設計上の抑制策(構造層による対象絞込、用件のターン帰属を使った入力削減、判断列限定の 2 run、安定列の蒸留)と、第 1 段 300 件で実測する項目を定める。
